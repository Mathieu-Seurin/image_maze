...
class VisionFilmGen(nn.Module):
    ...
    def __init__(self, num_features_per_block, num_resblock_to_modulate, num_channel_in,
        ...
        tmp = Variable(torch.zeros((1, self.num_channel_in,) + input_shape_per_channel))
        conv_out_shape = self.layer2(self.layer1(tmp)).shape
        n_values = conv_out_shape[1]*conv_out_shape[2]*conv_out_shape[3]

        self.fc_gammas = nn.Linear(n_values, self.num_features_to_modulate)
        self.fc_betas = nn.Linear(n_values, self.num_features_to_modulate)
...
class FilmedNet(nn.Module):
    def __init__(self, config, n_actions, state_dim, is_multi_objective):
        ...
        # Size of the feature maps, not their number
        self.input_shape = (state_dim['env_state'][1], state_dim['env_state'][2])
        ...
        if is_multi_objective and self.use_film:
            self.film_gen = VisionFilmGen(num_features_per_block=self.num_channel_per_state,
                                          ...
                                          input_shape_per_channel=self.input_shape)
