...
class ImageGridWorld(object):
    ...
    def render(self):
        ...
        """
        #Todo : don't recreate the plotted grid everytime, just change the point position
    ...
    def load_random_image_per_class(self, class_id, background_color, show=False):
        ...
        image_selected_channel_last = dataset[random_image_id]

        #black_area = np.where(image_selected == 0)
        black_area = np.where(image_selected_channel_last < 10)

        image_selected_channel_last = to_rgb_channel_last(image_selected_channel_last)
        ...
        return channel_last_to_channel_first(image_selected_channel_last)
