...
class ImageFmapGridWorld(object):
    ...
    def load_image_or_fmap(self, folder=None, class_id=None, seed=None, preproc=None):
        ...
        if preproc:
            return torch.load(path_to_images + '/{}'.format(chosen_file)).cuda()
        else:
            return ToTensor()(Image.open(path_to_images + '/{}'.format(chosen_file))).cuda()
    ...
    def get_current_square(self, preproc=False):
        x,y = self.position
        if preproc:
            return FloatTensor(self.grid_preprocessed[x,y])
        else:
            return FloatTensor(self.grid[x,y])
        ...
        if grid_type == "sequential":
            ...
            for i in range(self.n_row):
                for j in range(self.n_col):
                    ...
                    raw_img = self.load_image_or_fmap(folder='maze_images/{}' ,class_id=count, seed=plop, preproc=False)
